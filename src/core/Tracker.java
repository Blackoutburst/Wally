...
public class Tracker {
	...
	private void onHighscore(String currentQualification, String qualification, String currentFinals, String finals,
		...
		if (Integer.valueOf(currentQualification) < Integer.valueOf(qualification)) {
			server.getTextChannelById(channelID).sendMessage(Reader.read(Lines.qualifiers_score).replace("%player%", user).replace("%score%", qualification)).complete();
		...
		if (Integer.valueOf(currentFinals) < Integer.valueOf(finals)) {
			server.getTextChannelById(channelID).sendMessage(Reader.read(Lines.finals_score).replace("%player%", user).replace("%score%", finals)).complete();
